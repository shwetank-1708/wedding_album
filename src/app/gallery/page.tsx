...
export default function GalleryPage() {
    ...
    const fetchEvents = async () => {
        if (!user) return;
        try {
            const userEvents = await getUserEvents(user.email || user.name);
                        ...
                        {events.map((event, index) => (
                            ...
                            >
                                <Link
                                    ...
                                    className="group block relative aspect-[4/5] rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-stone-100"
                                >
                                    <Image
